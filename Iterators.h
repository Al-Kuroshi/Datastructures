...
// R
//  file ->  r script -> tools -> install packages
//	ggplot2
...
namespace sl
{
	...
	struct SL_List_Iterator
	{
		...
		explicit SL_List_Iterator(NodePtr<DataT> start = nullptr) : current(start)									{	}
		SL_List_Iterator(const SL_List_Iterator& src) : current(src.current)										{	}

		reference operator*()  const																				{ return current->data; }
		pointer operator->() const																					{ return &(operator*()); }

		bool operator==(SL_List_Iterator const& rhs) const															{ return current == rhs.current; }
		bool operator!=(SL_List_Iterator const& rhs) const															{ return current != rhs.current; }

		SL_List_Iterator& operator=(SL_List_Iterator const& src)													{ if (this == &src) { return *this; } current = src; return *this; }

		SL_List_Iterator& operator++()																				{ current = current->next; return *this; }

		SL_List_Iterator operator++(int)																			{ SL_List_Iterator preincremented = *this; current = current->next; return preincremented; }
	...
	struct const_SL_List_Iterator
	{
		...
		explicit const_SL_List_Iterator(NodePtr<DataT> start = nullptr) : current_node_ptr(start)					{	}
		const_SL_List_Iterator(const const_SL_List_Iterator& src) : current_node_ptr(src.current_node_ptr)			{	}

		const reference operator*()  const																			{ return current_node_ptr->data; }
		const pointer operator->() const																			{ return &(operator*()); }

		bool operator==(const const_SL_List_Iterator const& rhs) const												{ return current_node_ptr == rhs.current_node_ptr; }
		bool operator!=(const const_SL_List_Iterator const& rhs) const												{ return current_node_ptr != rhs.current_node_ptr; }


		const_SL_List_Iterator& operator++()																		{ current_node_ptr = current_node_ptr->next; return *this; }

		const_SL_List_Iterator operator++(int)																		{ const_SL_List_Iterator preincremented = *this; current_node_ptr = current_node_ptr->next; return preincremented; }
